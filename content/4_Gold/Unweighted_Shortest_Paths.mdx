...
def inside(x, y):
    # check if the cell is inside the grid and not '.'
    return (x > -1 and x < n and y > -1 and y < m and snow[x][y] != '.')

n, m = map(int, input().split())
snow = [input().strip() for _ in range(n)]

depth = [[0] * m for _ in range(n)]
ans = 1

# initialize deque with starting cell
q = deque()
q.append((0, 0))
depth[0][0] = 1
...
while q:
    c = q.popleft()
    ans = max(ans, depth[c[0]][c[1]])

    for i in range(4):
        x = c[0] + dx[i]
        y = c[1] + dy[i]
        if inside(x, y) and depth[x][y] == 0:
            if snow[x][y] == snow[c[0]][c[1]]:
                depth[x][y] = depth[c[0]][c[1]]
                q.appendleft((x, y))
            else:
                depth[x][y] = depth[c[0]][c[1]] + 1
                q.append((x, y))
...
If the question asks for 4000 * 4000 $\mathcal (m * n)$ it might TLE unless you optimize or use PyPy.
