...
```python
dp = [[[[ -1 for _ in range(2)] for _ in range(2)] for _ in range(50)] for _ in range(19)]
num = ""
...
def solve_dp(pos, k, under, started, targ, targ2):
    ...
    if pos == len(num):
        ...
        # if we are interested in finding out whether the sequence contains exactly half digits of targ and half digits of targ2.
        if targ2 != -1:
            if k == 20:
                return 1
            else:
        ...
        # otherwise, the sequence is interesting if at least half of the digits are our target number targ.
        if k >= 20:
            return 1
        else:
            return 0
    ...
    for i in range(10):
        ...
        # the sequence has started if any digits until pos is other than 0
        is_started = started | (1 if i != 0 else 0)
        if is_started == 1 and targ2 != -1 and i != targ and i != targ2:
            continue

        # the count for target digit targ is increased by one if i is targ, or decreased by one otherwise.
        new_k = k
        if is_started == 1:
            if i == targ:
                new_k = k + 1
...
def count_interesting_to(ubound):
    global num # usually you would want to avoid using global but in this case its fine
    num = str(ubound)
    ans = 0
    for i in range(10):
        reset()
...
with open("odometer.in") as f:
    X, Y = map(int, f.readline().split())

with open("odometer.out", "w") as f:
    f.write(str(count_interesting_to(Y) - count_interesting_to(X - 1)) + "\n")
```
